//
//  ConfirmNewOrderUseCaseImpl.swift
//  ZupZupManager
//
...
final class ConfirmNewOrderUseCaseImpl: ConfirmNewOrderUseCase {
    ...
    func run(_ carts: [CartEntity], 
        ...
        return try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<OrderEntity, Error>) in
            ...
            // TODO: Add Mapper
            let dto = ConfirmNewOrderDTO(
                orderList: carts.map {
                    ConfirmNewOrderDTO.Order(
                        itemId: $0.itemId,
                        itemName: $0.name,
                        imageUrl: $0.imageUrl,
                        itemPrice: $0.price,
            ...
            self.service.confirmNewOrder(dto, for: orderId, at: storeId)
                .sink { completion in
                    // TODO: Error Handling
