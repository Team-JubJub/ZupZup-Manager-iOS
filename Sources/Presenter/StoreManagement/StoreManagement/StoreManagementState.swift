...
struct StoreManagement {
    ...
    @Dependency(\.storeManagementClient) var storeManagementClient
    ...
    enum Action {
        ...
        case tapExitService                                     // 회원탈퇴 버튼을 눌렀을 경우
        ...
        case openStoreResponse(Result<OpenStoreResponse, Error>)   // 가게 ON/OFF API 호출의 결과
        case fetchStoreResponse(Result<StoreEntity, Error>)       // 가게 정보 조희 API 호출의 결과
    ...
    var body: some Reducer<State, Action> {
        ...
        Reduce { state, action in
            ...
            case .fetchStore: // 가게 정보 조희 API 호출
                ...
                return .run { send in
                    await send (
                        .fetchStoreResponse(Result {
                            try await self.storeManagementClient.fetchStore()
                        })
                    )
                }
            ...
            case .tapCustomerCenterButton: // 고객센터 버튼을 누른 경우
                state.isShowingCustomerCenter = true
                return .none
            ...
            case .tapStoreAlertOk: // 가게 ON/OFF Alert - 네 누른 경우
                ...
                let request = OpenStoreRequest(openOrClose: !state.storeEntity.isOpen)
                
                return .run { send in
                    await send (
                        .openStoreResponse(Result {
                            try await OpenStoreRepositoryImpl().opnStore(request: request)
                        })
                    )
                }
            ...
            case .dismissCustomerCenter:
                state.isShowingCustomerCenter = false
                return .none
...
struct StoreManagementClient {
    var fetchStore: () async throws -> StoreEntity
    var openStore: (OpenStoreRequest) async throws -> OpenStoreResponse
}

extension StoreManagementClient: DependencyKey {
    static let liveValue = StoreManagementClient(
        fetchStore: {
            // TODO: Fix this Code
            return try await FetchStoreRepositoryImpl().fetchStore().toStoreEntity()
        },
        openStore: { request in
            return try await OpenStoreRepositoryImpl().opnStore(request: request)
        }
    )
}

extension DependencyValues {
    var storeManagementClient: StoreManagementClient {
        get {self[StoreManagementClient.self]}
        set {self[StoreManagementClient.self] = newValue}
    }
}
